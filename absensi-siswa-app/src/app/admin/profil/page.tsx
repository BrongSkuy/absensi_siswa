...
export default function AdminProfilPage() {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <PageHeader title="Profil Administrator" description="Kelola detail profil pribadi dan keamanan akun" icon={User} />
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex flex-col space-y-1.5">
              <Label className="text-muted-foreground">Nama Admin</Label>
              <div className="font-semibold text-lg">{session?.user?.name || "-"}</div>
            </div>
            <div className="flex flex-col space-y-1.5">
              <Label className="text-muted-foreground">Username / Email</Label>
              <div className="font-mono bg-muted py-1 px-2 rounded w-fit">
                {(session?.user as Record<string, unknown>)?.username as string || session?.user?.email || "-"}
              </div>
            </div>
            <div className="flex flex-col space-y-1.5">
