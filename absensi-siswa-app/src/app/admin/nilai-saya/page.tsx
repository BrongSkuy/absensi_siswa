...
export default function AdminNilaiSayaPage() {
  ...
  useEffect(() => {
    fetch("/api/classes")
      .then((res) => res.json())
      .then((data: ClassRow[]) => setClasses(data || []))
  ...
  useEffect(() => {
    ...
    fetch(`/api/students?kelas=${encodeURIComponent(selectedKelas)}`)
      .then((res) => res.json())
      .then((resData: StudentRow[]) => {
  ...
  const handleFetchScores = () => {
    ...
    setError(null);
    setData(null);
    fetch(`/api/students/${selectedStudent}/scores`)
      .then((res) => res.json())
      .then((result) => {
        if (result.error) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Nilai Saya (Admin View)"
        description="Pilih siswa untuk melihat rincian lengkap penilaian kriteria dan skor SPK mereka"
        icon={ClipboardList}
      />
          ...
          <div className="flex flex-col md:flex-row gap-4 items-end">
            <div className="space-y-2 flex-1">
              ...
              >
                ...
                <SelectContent>
                  {classes.map((c) => (
                    <SelectItem key={c.id} value={c.namaKelas}>
                      {c.namaKelas}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="space-y-2 flex-1">
              ...
              <Select value={selectedStudent || null} onValueChange={(v) => setSelectedStudent(v || "")} disabled={!selectedKelas}>
                <SelectTrigger className="w-full">
                  <SelectValue placeholder="Pilih Siswa...">
                    {selectedStudent
                      ? students.find((s) => s.id === selectedStudent)
                        ? `${students.find((s) => s.id === selectedStudent)?.nis} - ${
                            students.find((s) => s.id === selectedStudent)?.namaLengkap
                          }`
                        : "Siswa terpilih"
                      : undefined}
                  </SelectValue>
            ...
            <Button
              ...
              className="gap-2 bg-navy-600 hover:bg-navy-700 w-full md:w-auto"
            >
      ...
      {error && (
        <div className="flex h-64 flex-col items-center justify-center gap-2 text-muted-foreground bg-white border rounded-xl shadow-sm">
      ...
      {!loading && !error && data && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          {/* Score Summary Card */}
          <Card className="border-navy-100 bg-gradient-to-r from-navy-500 to-navy-700 text-white overflow-hidden relative shadow-lg">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <Badge className="bg-amber-500 text-navy-950 hover:bg-amber-400 border-none">
          ...
          {data.kehadiran && (
            <div className="grid gap-4 sm:grid-cols-5">
              ...
              <Card className="border-navy-200 bg-navy-50/50 shadow-sm">
                <CardContent className="py-4 text-center">
              ...
              >
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between">
                    ...
                    <Badge
                      variant="outline"
                      className="shrink-0 ml-2 text-[10px] font-semibold bg-white/50"
                    >
                      {c.bobot}%
                    </Badge>
                  </div>
                ...
                <CardContent>
                  <div className="flex items-end justify-between">
                    <div>
                      ...
                      <Badge
                        variant="outline"
                        className={`mt-1 text-[10px] bg-white/50 ${getScoreColor(c.nilaiRataRata)}`}
                      >
