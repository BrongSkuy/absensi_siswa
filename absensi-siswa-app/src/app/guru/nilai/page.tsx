...
export default function GuruNilaiPage() {
  ...
  useEffect(() => {
    Promise.all([
    ...
    ]).then(([classesData, subjectsRes, criteriaData]) => {
      ...
      // API returns { subjects, isWaliKelas, waliClasses }
      const subjectsData = subjectsRes?.subjects || (Array.isArray(subjectsRes) ? subjectsRes : []);
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Input Nilai Multi-Kategori" description="Tentukan kategori nilai yang digunakan, input nilainya, dan hasil akhir SPK akan menggunakan reratanya." icon={PenLine} />
